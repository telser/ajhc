...
createSelectors sloc ds = mapM g ns where
    ...
    g (n,cs) = do
        ...
        pure $ HsFunBind (map f cs ++ [els]) where
...
runRename doit opt mod fls ns m = mapM_ addWarning errors >> pure (renamedMod,reverseMap) where
...
renameModule opt fls ns m = runRename go opt (hsModuleName m) fls (ns ++ driftResolvedNames) m
  where go mod = do
          ...
          pure (hsModuleDecls_u (++ inst) rmod,inst)
...
renameHsDecl d = f d where
    ...
    f (HsClassAliasDecl srcLoc name args hsContext hsClasses hsDecls) = do
        ...
        hsDecls' <- rename hsDecls
        pure (HsClassAliasDecl srcLoc name' args' hsContext' hsClasses' hsDecls')
    f (HsInstDecl srcLoc classHead hsDecls) = do
        ...
        pure (HsInstDecl srcLoc classHead' hsDecls')
    ...
    f prules@HsPragmaSpecialize { hsDeclSrcLoc = srcLoc, hsDeclName = n, hsDeclType = t } = do
        ...
        updateWith t $ do
            ...
            pure prules { hsDeclUniq = (m,i), hsDeclName = n, hsDeclType = t }
...
instance Rename HsRule where
    rename prules@HsRule { hsRuleSrcLoc = srcLoc, hsRuleFreeVars = fvs, hsRuleLeftExpr = e1, hsRuleRightExpr = e2 } = do
        ...
        m <- getCurrentModule
        i <- newUniq
        pure prules {  hsRuleUniq = (m,i), hsRuleFreeVars = fvs', hsRuleLeftExpr = e1', hsRuleRightExpr = e2' }
...
instance Rename HsConDecl where
    ...
    rename cd@(HsConDecl {  hsConDeclName = hsName, hsConDeclConArg = hsBangTypes, .. }) = do
        ...
        pure cd { hsConDeclName = hsName', hsConDeclConArg = hsBangTypes', hsConDeclExists }
    rename cd@HsRecDecl { hsConDeclSrcLoc = srcLoc, hsConDeclName = hsName, hsConDeclRecArg = stuff} = do
        ...
        stuff' <- sequence [ do ns' <- mapM renameName (map (toName FieldLabel) ns); t' <- rename t; pure (ns',t')  |  (ns,t) <- stuff]
        pure cd { hsConDeclName = hsName', hsConDeclRecArg = stuff', hsConDeclExists = es }
...
renameKindHsCon HsConDecl { .. } = do
    ...
    let bt e@HsBangedTy {} = do
        ...
        f e = addWarn InvalidDecl "invalid argument in kind declaration" >> pure e
    ...
    Just t -> do
        let f (HsPFieldPat x p) = case  mlookup (toName FieldLabel x) fls of
                ...
                Just cs -> case lookup n [ (nameName x,(y)) | (x,y) <- cs ] of
                    ...
                    Just i -> pure (i,HsPParen p)
        fm <- mapM f us
        let g i | Just e <- lookup i fm = pure e
                | otherwise = do
                    v <- newVar
                    pure $ HsPVar v
        rs <- mapM g [0 .. t - 1 ]
...
instance Rename HsExp where
    ...
    rename i@(HsLit HsInt {}) = do pure i
    rename i@(HsLit HsFrac {}) = do
        z <- rename f_fromRational
        pure $ HsParen (HsApp z i)
...
createError et s = do
    sl <- getSrcLoc
    pure $ HsError { hsExpSrcLoc = sl, hsExpErrorType = et, hsExpString = (show sl ++ ": " ++ s) }
    ...
    case mlookup (toName DataConstructor n) amp of
        ...
        Just t -> do
            let f (HsFieldUpdate x e) = case  mlookup (toName FieldLabel x) fls of
                    ...
                    Just cs -> case lookup n [ (nameName x,(y)) | (x,y) <- cs ] of
                        Nothing -> failRename $ "Field Label does not belong to constructor: " ++ show (x,n)
                        Just i -> pure (i,hsParen e)
            fm <- mapM f us
            let rs = map g [0 .. t - 1 ]
                g i | Just e <- lookup i fm = e
                    | otherwise = undef
            pure $ foldl HsApp (HsCon n) rs
...
buildRecUpdate (FieldMap amp fls) n us = do
        ...
        let f (HsFieldUpdate x e) = case  mlookup (toName FieldLabel x) fls of
                Nothing -> failRename $ "Field Label does not exist: " ++ show x
                Just cs -> pure [ (x,(y,hsParen e)) | (x,y) <- cs ]
        ...
        let g (c,zs) = case mlookup c amp of
                ...
                Just t -> do
                    ...
                    pure $ HsAlt sl (HsPApp c' (map HsPVar vars))  (HsUnGuardedRhs x) []
        as <- mapM g fm'
        pe <- createError HsErrorRecordUpdate "Record Update Error"
        pure $ HsCase n (as ++ [HsAlt sl HsPWildCard (HsUnGuardedRhs pe) []])
...
renameName hsName
    | Just n <- fromQuotedName hsName = pure n
    | hsName `elem` [tc_Arrow,dc_Unit,tc_Unit] = pure hsName
    | (nt,Just m,i) <- nameParts hsName, '@':_ <- show m = pure $ toName nt (m, i)
    | Just _ <- V.fromTupname hsName = pure hsName
...
collectDefsHsModule m = (\ (x,y) -> (Seq.toList x,Seq.toList y)) $ execWriter (mapM_ f (hsModuleDecls m)) where
    ...
    tellF xs = tell (Seq.fromList xs,Seq.empty) >> pure ()
    tellS xs = tell (Seq.empty,Seq.fromList xs) >> pure ()
...
instance DeNameable HsExp where
    deName mod e = f e where
        ...
        f p = runIdentity $ traverseHsExp (pure . dn) p
--        f (HsDo hsStmts) = do
--        (ss,()) <- renameHsStmts hsStmts (pure ())
--        doToExp newVar (nameName v_bind) (nameName v_bind_) (nameName v_fail) ss
--    rename (HsRecConstr hsName hsFieldUpdates) = do
