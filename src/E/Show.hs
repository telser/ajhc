...
showLit showBind l = do
    let f (LitInt i (ELit LitCons { litName = n })) | Just l <- lookup n enumList, i >= 0 && fromIntegral i < length l =
            pure $ atom $ ((text $ l !! (fromIntegral i)))
        f (LitInt n (ELit LitCons { litName = t})) | t == tc_Char_ = pure $ atom $ tshow (chr $ fromIntegral n) <> char '#'
        f (LitInt i t) | dump FD.EVerbose = do
            se <- showE t
            pure $ (atom (text $ show i) `inhabit` se )
        f (LitInt i _) = pure $ atom $ ((text $ show i))
        ...
        f LitCons { litName = s, litArgs = es } | Just n <- fromUnboxedNameTuple s, n == length es = do
            es' <- mapM (fmap unparse . showBind) es
            pure $ atom $ encloseSep (text "(# ") (text " #)") (text ", ") es'
        ...
        f LitCons { litName = n, litArgs = [e] } | tc_List == n = do
            e <- showBind e
            pure $  atom   (char '[' <> unparse e  <> char ']')
        f LitCons { litName = n, litArgs = [] } | dc_EmptyList == n = pure $ atom $ text "[]"
--        f LitCons { litName = n, litArgs = [] } | Just m <- getModule n, m `elem`[toModule "Jhc.Prim.Bits", toModule "Jhc.Prim.Word"]  = pure $ atom $ text "[]"
--        f LitCons { litName = ((tc_Addr_ ==) -> True), litType = ((eHash ==) -> True) } = pure $ atom $ text "Addr_"
--        f LitCons { litName = ((tc_FunAddr_ ==) -> True), litType = ((eHash ==) -> True) } = pure $ atom $ text "FunAddr_"
--        f LitCons { litName = ((tc_Char_ ==) -> True), litType = ((eHash ==) -> True) } = pure $ atom $ text "Char_"
...
--          where go n = do
--                    se <- showBind v
--                    pure $ atom (text n) `app` se
        f LitCons { litName = s, litArgs = es, litType = t,
		    litAliasFor = Just af } | dump FD.EAlias = do
	    s <- pure $ fromMaybe s (shortenName s)
            es' <- mapM showBind es
            se <- showE af
            pure $ foldl appCon (atom (tshow s <> char '@' <> parens (unparse se))) es' -- `inhabit` prettye t
        f LitCons { litName = s, litArgs = es, litType = t } = do
	    s <- pure $ fromMaybe s (shortenName s)
            es' <- mapM showBind es
            pure $ foldl appCon (atom (tshow s)) es' -- `inhabit` prettye t
        cons = bop (R,5) (text ":")
	shortenName n = Map.lookup n shortName `mplus` (getModule n >>= mm) where
	    mm m = if m `elem` shortMods then pure (toUnqualified n) else Nothing
...
showE e = do
    let f e | Just s <- E.E.toString e = pure $ atom $ (text $ show s)
        f e | Just xs <- eToList e = do
            xs <- mapM (fmap unparse . showE) xs
            pure $ atom $ list xs
        f e | e == tRational = pure $ atom $ text "Rational"
        f e | e == tString   = pure $ atom $ text "String"
        f e | e == tUnit     = pure $ atom $ text "()"
        f e | e == tWorld__  = pure $ atom $ text "RealWorld_"
        f e | e == vUnit     = pure $ atom $ text "()"
        ...
        f e0 | (as@(_:_), e) <- collectAbstractions e0 =
            foldr (\(_, tvr, _) -> allocTVr tvr)
                  (do tops <- mapM p as
                      e <- showE e
                      pure (fixitize (N,1) $ atom $ group $ (align $ skipToNest <> fillCat tops) <$> unparse e))
                  as
            where
              p :: (Doc, TVr, Bool) -> SEM Doc
              p (c,t,detailed) = do tvr <- if detailed then showTVr t else showTVr' t
                                    pure (c <> unparse tvr <> (char '.'))
        f (EVar tvr) = if dump FD.EVerbose then showTVr tvr else showTVr' tvr
        f Unknown = pure $ symbol (char  '?')
        f (ESort s) = pure $ symbol (tshow s)
        f (ELit (LitCons { litName = n, litArgs = [ELit (LitInt i _)] })) | n == dc_Char = pure $ atom $ tshow $ chr (fromIntegral i)
        f (ELit l) = showLit showE l
        f (EError "" t) = do
            ty <- showE t
            pure $ atom $ angles (text "exitFailure"  <>  UC.coloncolon <> unparse ty)
        f (EError s t) = do
            ty <- showE t
            pure $ atom $ angles ( UC.bottom <> char ':' <> text s <>  UC.coloncolon <> unparse ty)
        f (EPrim s es t) = do
            es' <- mapM showE es
            t <- showE t
            pure $ atom $ angles $ unparse $ foldl app (atom (pprint s)) es' `inhabit` t
        f ELetRec { eDefs = ds, eBody = e } = foldr (\(tvr,_) -> allocTVr tvr) (do
            e <- fmap unparse $ showE e
            ds <- mapM (fmap unparse . showDecl) ds
            pure $ fixitize (N,98) $ atom $ nest 2 (group ( keyword "let"
        ...
        f ec@(ECase { eCaseScrutinee = e, eCaseAlts = alts }) = mt (showE (eCaseType ec)) $  allocTVr (eCaseBind ec) $ do
            ...
            dcase <- case (eCaseDefault ec) of
                ...
                Just e -> do
                    e <- showE e
                    pure [unparse db <+> UC.rArrow </> unparse e]
        ...
        showAlt (Alt l e) = foldr allocTVr ans (litBinds l) where
            ans = do
                l <- showLit showTVr l
                e <- showE e
                pure $ unparse l <+> UC.rArrow </> unparse e
        showDecl (t,e) = do
            t <- subSEM $ showTVr t
            e <- subSEM $ showE e
            pure $ atom $ nest 2 $ group $ unparse t <+> (char '=') </> unparse e
        keyword x = text x
        symbol x = atom x
